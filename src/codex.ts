...
export async function resolveCodexBin(env: NodeJS.ProcessEnv = process.env): Promise<string> {
...
export async function runCodexLogout(codexBin: string, codexHome: string, cwd: string): Promise<void> {
  await runCodex(codexBin, ['logout'], codexHome, cwd, 'inherit');
}

export async function runCodexLogin(codexBin: string, accountHome: string, cwd: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on('exit', (code) => {
      if (code === 0) {
        ...
        reject(new Error(`codex ${args.join(' ')} 失败，退出码 ${code ?? 'unknown'}。`));
      }
    });
  });
}
...
async function runBrowserLoginWithoutOpeningBrowser(codexBin: string, codexHome: string, cwd: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      finish(() => reject(new Error('登录等待超时，请重新运行 cxa new。')));
    }, 15 * 60 * 1000);
    ...
    child.stdout.on('data', (chunk: Buffer) => {
      ...
      if (!loginStarted && loginMessage !== undefined) {
        ...
        if (loginMessage.error !== undefined) {
          finish(() => reject(new Error(formatAppServerFailure('启动登录失败', output, loginMessage))));
          return;
        }
        const authUrl = extractAuthUrl(loginMessage.result);
        if (authUrl === null) {
          finish(() => reject(new Error(formatAppServerFailure('启动登录失败：没有收到登录链接', output, loginMessage))));
          return;
        }
    ...
    child.on('exit', (code) => {
      ...
      reject(new Error(output.trim() || `codex app-server 退出码 ${code ?? 'unknown'}，登录未完成。`));
    });
...
function extractAuthUrl(value: unknown): string | null {
  if (typeof value !== 'object' || value === null || !('authUrl' in value)) return null;
  const authUrl = (value as { authUrl?: unknown }).authUrl;
  return typeof authUrl === 'string' && authUrl.length > 0 ? authUrl : null;
}

function formatAppServerFailure(title: string, output: string, message: JsonRpcMessage): string {
  const details = [title];
...
export async function prepareAcpHome(options: {
  ...
  if (await pathExists(sessions)) {
    await symlink(sessions, path.join(acpHome, 'sessions'), 'dir').catch(() => undefined);
  } else {
...
export async function activateAuth(authPath: string, codexHome: string): Promise<void> {
